#!/usr/bin/env python3
"""Generate SCOPE pre-LPM intent-stratified descriptive diagnostics."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--output-dir", type=Path, default=BASE / "tables/pre_lpm_eda_v6_intent_stratified")
    parser.add_argument("--figure-dir", type=Path, default=BASE / "figures/pre_lpm_eda_v6_intent_stratified")
    args = parser.parse_args(argv)
    if not args.lpm.exists():
        parser.error(f"LPM-ready CSV not found: {args.lpm}")
    print(json.dumps(run_intent_stratified_diagnostics_v6(args.lpm, args.eda if args.eda.exists() else None, args.output_dir, args.figure_dir), indent=2))
    return 0

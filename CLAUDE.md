...
11. **Confounder & proxy framework** (Iteration L, branch `econometrics-layer`) — NEW `src/confounders.py`: a **32-entry confounder registry** (bias mechanism, availability, model/diagnostic role, caveat) + `derive_proxy_features` (no-scrape **proxies**: CiteScope-observed domain/url visibility counts, `citescope_visibility_history_score`, URL semantics, prompt-wording flags, language/local-relevance heuristics, grouped content/trust scores — every one labelled a *proxy*, not the true construct) + `confounder_audit` (registry / feature-availability / proxy-summary / cited-vs-more-only balance / correlation-matrix / proxy-VIF / **unmeasured-confounders**). Confounder-aware **sensitivity Models E–H** in `model_comparison` (Model-D focal + proxy **controls**; **never the headline** — Model C stays headline). 7 confounder exports (6 CSV + `econometrics_unmeasured_confounders.md`) + `report._confounder_section` + `confounder_audit` JSON block; `components.sensitivity_block` confounder render; `config.py` proxy / price-association / source-availability caveats; `tests/test_confounders.py` (10 tests).
12. **Page-type-stratified + inference safeguards** (Iteration M, branch `econometrics-layer`) — **inference rule change:** the **wild cluster bootstrap is now sensitivity-only** — `fit_citation_model` keeps the **analytic HC3/cluster SE as the headline** (never overwritten), reports each focal coef's `se_wild_bootstrap` + a `recommended_inference`, and always emits a few-cluster warning under 40 clusters. NEW `econometrics.page_type_stratified_analysis` runs **one independent LPM per major `page_type`** (row stays one surfaced source appearance; `page_type` is never a dummy or a cluster inside the subgroup; gates `min_n=50`/`min_cited=10`/`min_more_only=10`, skip-diagnostics) → `page_type_stratified_summary/coefficients/warnings` CSVs. NEW `econometrics.inference_sensitivity` reports SE under **HC3 / cluster(domain) / cluster(prompt_id) / two-way domain×prompt_id / wild-bootstrap** + `recommended_inference` → `econometrics_inference_sensitivity.csv`. `report._stratified_section` + JSON block + `components.sensitivity_block` render + ChatGPT Report-tab buttons; `config.py` PAGE_TYPE_MIN_* gates; `docs/econometrics_human_decision_guide.md` §4.1–4.7 edits; rewrote the wild-bootstrap test + 4 new tests. _(latest)_
...
`_sensitivity_section`; new `config.py` caveats) **plus the Iteration L confounder framework** (NEW
`src/confounders.py`: 32-entry confounder registry + `derive_proxy_features` (no-scrape proxies:
CiteScope-observed domain/url visibility, URL/prompt/language/local heuristics, content/trust scores) +
`confounder_audit` (registry / availability / proxy-summary / balance / correlation / VIF / unmeasured);
confounder-aware **sensitivity Models E–H** in `model_comparison` (D-focal + proxy CONTROLS, never the
headline); 7 confounder exports + `report._confounder_section` + `confounder_audit` JSON block;
`components.sensitivity_block` confounder render; `config.py` proxy/price/source-availability caveats;
`tests/test_confounders.py`) **plus the Iteration M page-type-stratified + inference-safeguards upgrade**
(**wild bootstrap → sensitivity-only**: never overwrites the headline analytic cluster SE; per-coef
`se_wild_bootstrap` + `recommended_inference`; NEW `econometrics.page_type_stratified_analysis` = one
independent LPM per `page_type` with min-n/cited/more-only gates, no page_type dummy/cluster inside a
subgroup; NEW `econometrics.inference_sensitivity` = SE under HC3/domain/prompt_id/two-way/wild; 4 new
exports + `report._stratified_section` + JSON + UI; `config.py` PAGE_TYPE_MIN_*;
`docs/econometrics_human_decision_guide.md` §4.1–4.7; `tests/test_econometrics.py` +4).
**92 pytest tests pass; AppTest renders both modes incl. the regression + sensitivity + confounder + page-type-stratified sections; verified end-to-end on a real 1,270-source ChatGPT audit.** Untracked reference files

...
### Confounder & proxy framework · branch `econometrics-layer` (latest)

**Goal.** Reduce omitted-variable bias *where the data allows* — and be explicit and honest about what it
doesn't. New module `src/confounders.py`.

- **Registry (32 confounders).** `CONFOUNDER_REGISTRY` catalogues each confounder's bias mechanism,
  availability (`available_directly` / `available_heuristic` / `proxy_only` / `external_required` /
  `post_output_only` / `not_currently_available`), whether a proxy is possible, whether external data is
  required, the recommended columns, and its model/diagnostic role + caveat → `econometrics_confounder_registry.csv`.
- **Proxy derivation (no extra scraping).** `derive_proxy_features` computes ~40 observable proxies, each
  **labelled a proxy, never the true construct**: CiteScope-observed domain/url **visibility** counts +
  `citescope_visibility_history_score` (explicitly *not* `index_history_score`), **URL semantics**,
  **prompt-wording** flags, **language / local-relevance** heuristics (bilingual EN/TH), and grouped
  **content/trust** scores. Missing columns are skipped with a note — never a crash.
- **Audit (7 outputs).** `confounder_audit` → registry · **feature-availability** (present? derived?
  missing-rate) · **proxy-summary** (quality: strong/moderate/weak/external/post-output) · **balance**
  cited-vs-more-only (+ imbalance warning) · **correlation matrix** · **proxy VIF** ·
  **unmeasured-confounders** (`econometrics_unmeasured_confounders.md`).
- **Sensitivity Models E–H.** Same focal as Model D, each tier adds confounder-proxy **controls**
  (E prompt-wording + language/local · F + content completeness · G + visibility history · H +
  metadata/access). They show whether focal estimates **shrink** once proxies are adjusted for, and are
  **never the headline** — Model C stays the headline; the A/B/C/D ladder is untouched.
- **Bad-control safety.** Answer-derived / post-output features stay barred from every A–H model;
  `source_position` keeps the with/without + mediator caveat; `scrape_success` is a selection indicator.
- **Wording.** Proxies are proxies; `CiteScope-observed visibility ≠ external SEO authority`;
  visibility-history ≠ true index history; "citation **within surfaced sources**, not retrieval from the
  whole web." Report section "Confounder & Proxy Audit" + `confounder_audit` JSON block + 7 exports + UI.
- **Tests.** `tests/test_confounders.py` (10) → **88 total**. Verified on the real 1,270-source audit.
...
**Confounder & proxy audit (Update L) — 7 more:**

| File | Content |
|---|---|
| `econometrics_confounder_registry.csv` | 32 confounders × bias mechanism / availability / role / caveat |
| `econometrics_confounder_feature_availability.csv` | Per recommended column: available? derived? missing-rate |
| `econometrics_confounder_proxy_summary.csv` | Per confounder: available proxy features + proxy quality |
| `econometrics_confounder_balance_by_cited.csv` | Each proxy: cited vs more-only value + difference + warning |
| `econometrics_confounder_correlation_matrix.csv` | Correlation among numeric proxy features |
| `econometrics_confounder_vif.csv` | VIF of the confounder-proxy controls |
| `econometrics_unmeasured_confounders.md` | Measurable vs proxy-only vs external + business-safe caveats |

Plus the `confounder_audit` + `confounder_sensitivity_rows` (Models D→E→F→G→H) blocks in the JSON bundle.
